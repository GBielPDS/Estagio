...
function iconesNavegacao(): array
{
    return [
...
function sidebar(string $paginaAtiva = ''): void
{
    global $conn;
    ...
    $paginaAtivaFormatada = strtolower($paginaAtiva);
    $icones = iconesNavegacao();
    $totalAlertas = 0;

    if (isset($conn) && $conn instanceof mysqli && function_exists('buscarAlertasEstoque')) {
        $totalAlertas = count(buscarAlertasEstoque($conn));
    }

    $ativo = fn(string $pagina): string => $paginaAtivaFormatada === strtolower($pagina) ? ' aria-current="page"' : '';

    $icone = fn(string $nomeIcone): string => '<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">' . ($icones[$nomeIcone] ?? '') . '</svg>';
